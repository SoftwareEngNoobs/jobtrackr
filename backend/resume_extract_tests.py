...
class FlaskTest(unittest.TestCase):
    ...
    def test_complex_footer(self):
        self.assertTrue("REFERENCES AVAILABLE UPON REQUEST" in extract_text_from_pdf(
            os.path.join(path, "resume-sample1.pdf")))

    def test_numbers(self):
        self.assertTrue("4.00" in extract_text_from_pdf(
            os.path.join(path, "resume-sample1.pdf")))

    def test_headers(self):
        self.assertTrue("EDUCATION" in extract_text_from_pdf(
            os.path.join(path, "resume-sample1.pdf")))
